...
// CODE HERE
const totalPopulation = populations.reduce((acc, elem) => {
  return acc + elem
})
...
const bobsPurchases = purchases.filter(person => {
  if (person.owner === "Bob") {
    return person.price
  }
})

const bobsPurchases2 = bobsPurchases.map(person => {
  return person.price
})

const bobsTotal = bobsPurchases2.reduce((acc, elem) => {
  return acc + elem
})
